...
`),r=await I.readSidecar(this.document.uri.fsPath),n=[];if(r){let{updates:a,anchorsMoved:o}=Cr.detectStaleThreads(t,r.comments);for(let{thread:c,newStatus:m}of a)c.status=m;n=r.comments,(a.length>0||o)&&await I.writeSidecar(this.document.uri.fsPath,r,"internal")}n.sort((a,o)=>a.anchor.markdownRange.startOffset-o.anchor.markdownRange.startOffset);let s=n.map(a=>{let o=n.filter(c=>c.anchor.selectedText===a.anchor.selectedText&&c.anchor.markdownRange.startOffset<a.anchor.markdownRange.startOffset).length;return{id:a.id,selectedText:a.anchor.selectedText,occurrenceIndex:o,status:a.status,color:a.color,thread:a.thread,startOffset:a.anchor.markdownRange.startOffset}}),i=await ie.getUserName();this.panel.title=`Preview: ${oe.basename(this.document.uri.fsPath)}`,this.panel.webview.html=this.buildHtml(t,s,i)}finally{this._isUpdating=!1}}}buildHtml(t,r,n){let s=tl(),i=this.panel.webview.cspSource,a=JSON.stringify(r).replace(/</g,"\\u003c"),o=JSON.stringify(n).replace(/</g,"\\u003c"),c=oe.basename(this.document.uri.fsPath),m=this.docDirUri().toString(),f="null",l="",h="",R=w.workspace.getConfiguration("markdownThreads").get("enableSpecitRendering",!0)?Li(t):null;if(R){let k=Pi(this.document.uri.fsPath),T=Sr[k]??Sr.Other;f=JSON.stringify({fields:R.fields,editableFields:[...Fi],statusOptions:T}).replace(/</g,"\\u003c"),R.fields.Project&&(c=R.fields.Project);let L=Di(k);L&&(h=`
          <p class="doc-subtitle">${He(L)}</p>`);let re=R.fields.Status;if(re){let Fe=re.toLowerCase();l=`
